...
CHARACTER(LEN=1), DIMENSION(1:7)                        ::  M_Letters
...
DO L_Limit_Input = L_Limit_Min, L_Limit_Max
    ...
    Suffix_Tail = M_Letters(M_Index)



    WRITE(*,'(A,I4.4,A,I1.1,A,I2.2,A,I2.2)')" # RE       : ", NE(1),         &
    ...
    DO re = 1,NE(1)
        ...
        DO pq = 1,NQ(3)
            ...
            IF ( cur_r_locs(rq) .LE. Star_Radius ) THEN
                Local_E(q, re-1, 0, 0) = rho_o
    !            Print*,Local_E(rq, re-1, 0, 0)
            ELSE
                Local_E(q, re-1, 0, 0) = 0.0_idp
            END IF
