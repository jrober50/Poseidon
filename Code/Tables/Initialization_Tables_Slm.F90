...
DO pd = 1,Num_P_Quad_Points
    ...
    Slm_Elem_Values(lm,tpd) = Nlm_Values(short_lm)              &
                            * Plm_Values(td,short_lm,pe)        &
                            * Am_Values(pd,m,te)
                            
    Slm_Elem_dt_Values(lm,tpd) = Nlm_Values(short_lm)           &
                               * Plm_dt_Values(td,short_lm,pe)  &
                               * Am_Values(pd,m,te)
                               
    Slm_Elem_dp_Values(lm,tpd) = Nlm_Values(short_lm)           &
                               * Plm_Values(td,short_lm,pe)     &
                               * Am_dp_Values(pd,m,te)
...
CALL Init_Message('Intializing Slm Tables')
...
IF (L_Max > 0 ) THEN
    DO Elem = E_LoHi(1),E_LoHi(2)
        ...
        Cur_Locs = Map_From_X_Space(E_Locs(Elem), E_Locs(Elem + 1), QP_xLocs)
        DO Quad = 1,QP_Num

            Cosm(Quad,1,Elem) = DCOS(Cur_Locs(Quad))
            Sinm(Quad,1,Elem) = DSIN(Cur_Locs(Quad))
...
REAL(idp),  DIMENSION(1:QP_Num,0:1,0:E_Num-1)                       ::  cosm
REAL(idp),  DIMENSION(1:QP_Num,0:1,0:E_Num-1)                       ::  sinm
...
IF ( L_Max > 0 ) THEN
    ...
    DO m = 0,L_Max-2
        ...
        lfacta = REAL(2.0*l - 1.0_idp, KIND = idp)/REAL(l-m, KIND = idp)
        lfactb = REAL(l+m-1,KIND=idp)/REAL(l-m,KIND=idp)
