...
!IF (myID == 0 ) THEN
!  PRINT*,"Before CREATE_3D_NONLAPLACIAN_SOE"
!END IF
...
!Print*,"CREATE_3D_NONLAPLACIAN_STF_MAT :",timea-timeb,myID
...
!    PRINT*,"Before REDUCE_3D_NONLAPLACIAN_SOE",myID
!END IF
timeb = MPI_Wtime()
...
!Print*,"REDUCE_3D_NONLAPLACIAN_STF_MAT :",timea-timeb,myID
CALL Clock_In(timea-timeb, 10)
!if (POSEIDON_COMM_PETSC .NE. MPI_COMM_NULL) THEN
!    PRINT*,"AFTER REDUCE_3D_NONLAPLACIAN_SOE",myID
!END IF
...
IF (POSEIDON_COMM_PETSC .NE. MPI_COMM_NULL) THEN
!    PRINT*,"Before FINISH_3D_NONLAPLACIAN_MATRIX",myID
END IF
timeb = MPI_Wtime()
CALL FINISH_3D_NONLAPLACIAN_MATRIX()
timea = MPI_Wtime()

!Print*,"FINSIH_3D_NONLAPLACIAN_STF_MAT :",timeb-timea,myID
...
!       PRINT*,"Before FINISH RHS",myID
!       PRINT*,Block_RHS_Vector(START_HERE:END_HERE)
!       PRINT*," "
!  END IF
!  CALL SLEEP(1)
!  CALL MPI_BARRIER(POSEIDON_COMM_WORLD, ierr)
...
IF (POSEIDON_COMM_PETSC .NE. MPI_COMM_NULL) THEN
!    print*,"Before FINISH_3D_RHS_VECTOR",myID
END IF
timeb = MPI_Wtime()
...
!Print*,"   FINISH_3D_RHS_VECTOR :",timea-timeb,myID
...
!timeb = MPI_Wtime()
!CALL INSERT_3D_NONLAPLACIAN_STF_MAT_BLOCK()
!timea = MPI_Wtime()
!
!Print*,"INSERT_3D_NONLAPLACIAN_STF_MAT :",timea-timeb
...
!PRINT*,"STF_MAT 2"
!PRINT*,STF_MAT
!CALL WRITE_BDDtoFULL_MATRIX(STF_MAT, 3*NUM_OFF_DIAGONALS, PROB_DIM-1, 20+myID)
!CALL WRITE_BDDtoFULL_MATRIX_Block(BLOCK_STF_MAT, 2*NUM_OFF_DIAGONALS, Local_PROB_DIM-1, 50+myID)
...
!REAL(KIND = idp), DIMENSION( NUM_QUAD_DOF, 1:14 )           ::  SUBJCBN_PSI_TERMS_B

!REAL(KIND = idp), DIMENSION( NUM_QUAD_DOF, 1:14 )           ::  SUBJCBN_ALPHAPSI_TERMS_B

!REAL(KIND = idp), DIMENSION( NUM_QUAD_DOF, 1:20 )           ::  SUBJCBN_BETA1_TERMS_B

!REAL(KIND = idp), DIMENSION( NUM_QUAD_DOF, 1:20 )           ::  SUBJCBN_BETA2_TERMS_B

!REAL(KIND = idp), DIMENSION( NUM_QUAD_DOF, 1:20 )           ::  SUBJCBN_BETA3_TERMS_B


REAL(KIND = idp), DIMENSION( 1:14, 1:NUM_QUAD_DOF )           ::  SUBJCBN_PSI_TERMS_B

REAL(KIND = idp), DIMENSION( 1:14, 1:NUM_QUAD_DOF )           ::  SUBJCBN_ALPHAPSI_TERMS_B

REAL(KIND = idp), DIMENSION( 1:20, 1:NUM_QUAD_DOF )           ::  SUBJCBN_BETA1_TERMS_B

REAL(KIND = idp), DIMENSION( 1:20, 1:NUM_QUAD_DOF )           ::  SUBJCBN_BETA2_TERMS_B

REAL(KIND = idp), DIMENSION( 1:20, 1:NUM_QUAD_DOF )           ::  SUBJCBN_BETA3_TERMS_B



COMPLEX(KIND = idp), DIMENSION(1:6,                    &
                               1:NUM_T_QUAD_POINTS,    &
                               1:NUM_P_QUAD_POINTS,    &
...
COMPLEX(KIND = idp), DIMENSION(1:NUM_T_QUAD_POINTS,    &
                               1:NUM_P_QUAD_POINTS,    &
...
REAL(KIND = idp), DIMENSION(1:NUM_R_QUAD_POINTS, NUM_T_QUAD_POINTS)     ::  RSIN_SQUARE
...
REAL(KIND = idp), DIMENSION(-L_LIMIT:L_LIMIT, 0:L_LIMIT)                ::  Sqrt_Term
...
DO l = 1,L_LIMIT
    ...
    DO m = -M_VALUES(l),M_VALUES(l)
        ...
        Sqrt_Term(m,l) = SQRT((2*REAL_L + 1)/(2*REAL_L - 1)* REAL( (l-m)*(l+m), idp) )
...
!            CALL MPI_BARRIER(POSEIDON_COMM_WORLD,ierr)
!            IF (myID == 0) THEN
!               PRINT*,"Before CALC_3D_Current_Values_Block",myID
!            END IF
            timea = MPI_Wtime()
            ...
            IF (myID == 0 ) THEN
                PRINT*,A_FACTORS(1,1,1,:)-Ylm_Values(:, 1, 1, Local_te, Local_pe)
                PRINT*,"---------------------------------------------"
                PRINT*,A_FACTORS(2,1,1,:)-Ylm_dt_Values(:, 1, 1, Local_te, Local_pe)
                PRINT*,"---------------------------------------------"
                PRINT*,A_FACTORS(3,1,1,:)-Ylm_dp_Values(:, 1, 1, Local_te, Local_pe)
                PRINT*,"---------------------------------------------"
                PRINT*,A_FACTORS(4,1,1,:)-Ylm_dtt_Values(:, 1, 1, Local_te, Local_pe)
                PRINT*,"---------------------------------------------"
                PRINT*,A_FACTORS(5,1,1,:)-Ylm_dtp_Values(:, 1, 1, Local_te, Local_pe)
                PRINT*,"---------------------------------------------"
                PRINT*,A_FACTORS(6,1,1,:)-Ylm_dpp_Values(:, 1, 1, Local_te, Local_pe)
                PRINT*,"---------------------------------------------"
                PRINT*,CC_Ylm_Term(1,1,:)-Ylm_CC_Values(:, 1, 1, Local_te, Local_pe)
               PRINT*," "
               PRINT*," "
            END IF
...
!            CALL MPI_BARRIER(POSEIDON_COMM_WORLD,ierr)
!            IF (myID == 0) THEN
!                PRINT*,"Calc_3D_SubJcbn_Terms_Block",myID
!            END IF
...
!            CALL MPI_BARRIER(POSEIDON_COMM_WORLD,ierr)
!            IF (myID == 0 ) THEN
!                PRINT*,"CREATE_3D_RHS_VECTOR_BLOCK",myID
!            END IF
            CALL CREATE_3D_RHS_VECTOR(  Local_re ,                              &
                                        RHS_TERMS,                              &
                                        CC_Ylm_Term,                            &
                                        Int_Factor                              )
...
!            CALL MPI_BARRIER(POSEIDON_COMM_WORLD,ierr)
!            IF (myID == 0 ) THEN
!                print*,"CREATE_3D_JCBN_MATRIX_BLOCK",myID
!            END IF
...
COMPLEX(KIND = idp), INTENT(INOUT), DIMENSION( 1:6,                     &
                                               1:NUM_T_QUAD_POINTS,     &
                                               1:NUM_P_QUAD_POINTS,     &
                                               0:LM_LENGTH              )   :: A_FACTORS


COMPLEX(KIND = idp), INTENT(INOUT), DIMENSION( 1:NUM_T_QUAD_POINTS,     &
                                               1:NUM_P_QUAD_POINTS,     &
                                               0:LM_LENGTH              )   :: CC_Ylm_Term
...
REAL(KIND = idp), INTENT(IN), DIMENSION(1:NUM_R_QUAD_POINTS, NUM_T_QUAD_POINTS) ::  RSIN_SQUARE
...
COMPLEX(KIND = idp), INTENT(IN), DIMENSION(1:NUM_P_QUAD_POINTS)         ::  PHI_EXP,            &
                                                                            PHI_TWOEXP
...
REAL(KIND = idp), INTENT(IN), DIMENSION(-L_LIMIT:L_LIMIT, 0:L_LIMIT)    ::  SQRT_TERM
...
!$OMP           A_VALUE_R, A_VALUE_T, A_VALUE_P,                        &
!$OMP           A_VALUE_TT, A_VALUE_TP, A_VALUE_PP,                     &
...
!$OMP           SIN_VAL, SIN_SQUARE, COTAN_VAL, COS_VAL, COS_SQUARE,    &
!$OMP           PHI_EXP, PHI_TWOEXP,                                    &
!$OMP           SQRT_TERM,                                              &
!$OMP           myID,                                                   &
        ...
        DO rd = 1,NUM_R_QUAD_POINTS
            ...
            DO l = 0,L_Limit
                ...
                DO m = -M_VALUES(l),M_VALUES(l)
                    ...
                    A_VALUE_R = Ylm_Table_Block(m,l,td, pd, te, pe)

                    A_VALUE_T = l*COTAN_VAL(td)*Ylm_Table_Block(m,l,td,pd,te,pe)      &
                              - SQRT_TERM(m,l)*(1.0_idp/SIN_VAL(td))            &
                              * Ylm_Table_Block(m,l-1,td,pd,te,pe)


                    A_VALUE_P = CMPLX(0, m, idp)                                &
                                    * Ylm_Table_Block(m, l, td, pd, te, pe)



                    A_VALUE_TT = ((m*m-l)/SIN_SQUARE(td) - l*l)*Ylm_Table_Block(m,l,td,pd,te,pe)    &
                               + SQRT_TERM(m,l)*(1/SIN_VAL(td))*COTAN_VAL(td)                       &
                               * Ylm_Table_Block(m,l-1,td,pd,te,pe)


                    A_VALUE_TP = CMPLX(0,m,idp)*A_VALUE_T

                    A_VALUE_PP = CMPLX( 0, -m*m, idp)                               &
                                    * Ylm_Table_Block(m, l, td, pd, te, pe)
                    ...
                    A_FACTORS(1, td, pd, lm_loc ) = A_VALUE_R
                    A_FACTORS(2, td, pd, lm_loc ) = A_VALUE_T
                    A_FACTORS(3, td, pd, lm_loc ) = A_VALUE_P
                    A_FACTORS(4, td, pd, lm_loc ) = A_VALUE_TT
                    A_FACTORS(5, td, pd, lm_loc ) = A_VALUE_TP
                    A_FACTORS(6, td, pd, lm_loc ) = A_VALUE_PP

                    CC_Ylm_Term(td, pd, lm_loc) = M_POWER_TABLE(m)*Ylm_Table_Block(-m,l,td, pd, te, pe)
...
!                    END IF
                    ...
                    DO d = 0,DEGREE
                        ...
                        Local_Coefficients = Coefficient_Vector(Current_Location:Current_Location+4)
...
!       PRINT*,"JCBN_BIGK_VALUE",JCBN_BIGK_VALUE
        ...
        ! J_{2,4lmn}
        SUBJCBN_BETA1_TERMS(5:8, rd, td, pd) = -7.0_idp * PSI_POWER(1)                          &
        ...
        SUBJCBN_BETA1_TERMS(13, rd, td, pd) = -TwoThirds*CUR_R_LOCS(rd)* JCBN_n_ARRAY(2)

        ! J_{4,3lmn} Derivative Terms
        SUBJCBN_BETA1_TERMS(14, rd, td, pd) = -TwoThirds * R_SQUARE(rd) * JCBN_n_ARRAY(2)
        SUBJCBN_BETA1_TERMS(15, rd, td, pd) = (2.0_idp / (3.0_idp * R_CUBE(rd))) - JCBN_n_ARRAY(1)*(1.0_idp/R_SQUARE(rd))
        SUBJCBN_BETA1_TERMS(16, rd, td, pd) = 0.0_idp


        ! J_{5,3lmn} Non-Derivative Term
        SUBJCBN_BETA1_TERMS(17, rd, td, pd) = -TwoThirds*CUR_R_LOCS(rd)*SIN_SQUARE(td)* JCBN_n_ARRAY(3)

        ! J_{5,3lmn} Derivative Terms
        SUBJCBN_BETA1_TERMS(18, rd, td, pd) = -FourThirds * RSIN_SQUARE(rd, td) * JCBN_n_ARRAY(3)
        SUBJCBN_BETA1_TERMS(19, rd, td, pd) = 0.0_idp
        SUBJCBN_BETA1_TERMS(20, rd, td, pd) = OneThird * RSIN_SQUARE(rd, td) * (( 2.0_idp /CUR_R_LOCS(rd) )             &
        ...
        ! J_{2,4lmn}
        SUBJCBN_BETA2_TERMS(5:8, rd, td, pd) = -7.0_idp * PSI_POWER(1)                      &
                                                * SUBJCBN_BETA2_TERMS(1:4, rd, td, pd)



        ! J_{3,4lmn} Non-Derivative Term
        SUBJCBN_BETA2_TERMS(9, rd, td, pd) = TwoThirds*COTAN_VAL(td) * JCBN_n_ARRAY(1)

        ! J_{3,4lmn} Derivative Terms
        SUBJCBN_BETA2_TERMS(10, rd, td, pd) = OneThird * COTAN_VAL(td) - JCBN_n_ARRAY(2)
        SUBJCBN_BETA2_TERMS(11, rd, td, pd) = TwoThirds * JCBN_n_ARRAY(1)
        ...
        SUBJCBN_BETA2_TERMS(14, rd, td, pd) = -JCBN_n_ARRAY(1)
        SUBJCBN_BETA2_TERMS(15, rd, td, pd) = OneThird * R_SQUARE(rd) * ( COTAN_VAL(td) - 4.0_idp * JCBN_n_ARRAY(2) )
        SUBJCBN_BETA2_TERMS(16, rd, td, pd) = - (1.0_idp/RSIN_SQUARE(rd, td)) * JCBN_n_ARRAY(3)


        ! J_{5,4lmn} Non-Derivative Term
        SUBJCBN_BETA2_TERMS(17, rd, td, pd) = -FourThirds*R_SQUARE(rd)*SIN_VAL(td)*COS_VAL(td)* JCBN_n_ARRAY(3)

        ! J_{5,4lmn} Derivative Terms
        SUBJCBN_BETA2_TERMS(18, rd, td, pd) = 0.0_idp
        SUBJCBN_BETA2_TERMS(19, rd, td, pd) = -FourThirds * RSIN_SQUARE(rd, td) * JCBN_n_ARRAY(3)
        SUBJCBN_BETA2_TERMS(20, rd, td, pd) = OneThird * RSIN_SQUARE(rd, td) * ( COTAN_VAL(td) + 3.0_idp * JCBN_n_ARRAY(2)  )
        ...
        ! J_{2,5lmn}
        SUBJCBN_BETA3_TERMS(5:8, rd, td, pd) = -7.0_idp * PSI_POWER(1)                              &
        ...
        SUBJCBN_BETA3_TERMS(9, rd, td, pd) =  (2.0_idp/3.0_idp)*COTAN_VAL(td)* JCBN_n_ARRAY(1)

        ! J_{3,5lmn} Derivative Terms
        SUBJCBN_BETA3_TERMS(10, rd, td, pd) = -JCBN_n_ARRAY(3) + COTAN_VAL(td)/3.0_idp
        SUBJCBN_BETA3_TERMS(11, rd, td, pd) = 0.0_idp
        SUBJCBN_BETA3_TERMS(12, rd, td, pd) = -TwoThirds * JCBN_n_ARRAY(1)


        ! J_{4,5lmn} Non-Derivative Term
        SUBJCBN_BETA3_TERMS(13, rd, td, pd) = ( RSIN_SQUARE(rd,td)*SIN_VAL(td)*COS_VAL(td) - R_SQUARE(rd)*COTAN_VAL(td) )    &
                                    * JCBN_n_ARRAY(3)

        ! J_{4,5lmn} Derivative Terms
        SUBJCBN_BETA3_TERMS(14, rd, td, pd) = 0.0_idp
        SUBJCBN_BETA3_TERMS(15, rd, td, pd) = FourThirds * R_SQUARE(rd) * JCBN_n_ARRAY(3)
        SUBJCBN_BETA3_TERMS(16, rd, td, pd) = -FourThirds * R_SQUARE(rd) * JCBN_n_ARRAY(2)


        ! J_{5,5lmn} Non-Derivative Term
        SUBJCBN_BETA3_TERMS(17, rd, td, pd) = (R_CUBE(rd)*SIN_SQUARE(td)*SIN_SQUARE(td) - R_INVERSE(rd) )  &
                                    * JCBN_n_ARRAY(1)                                           &
                              + ( RSIN_SQUARE(rd, td)*SIN_VAL(td)*COS_VAL(td) - R_SQUARE(rd)*COTAN_VAL(td) )    &
                                    * JCBN_n_ARRAY(2)
        ! J_{5,5lmn} Derivative Terms
        SUBJCBN_BETA3_TERMS(18, rd, td, pd) = JCBN_n_ARRAY(1)
        SUBJCBN_BETA3_TERMS(19, rd, td, pd) = R_SQUARE(rd) * JCBN_n_ARRAY(2)
        SUBJCBN_BETA3_TERMS(20, rd, td, pd) = FourThirds * R_SQUARE(rd) * JCBN_n_ARRAY(3)
...
! DO td = 1,NUM_T_QUAD_POINTS
!  DO rd = 1,NUM_R_QUAD_POINTS
!    PRINT*,"RHS_TERMS",re,te,pe,rd,td,pd
!    PRINT*,RHS_TERMS(:,rd,td,pd)
!  END DO
! END DO
!END DO
...
!                                                                                !
!################################################################################!
SUBROUTINE CREATE_3D_RHS_VECTOR(    re,                                     &
                                    RHS_TERMS,                              &
                                    CC_Ylm_Term,                            &
                                    Int_Factor                              )
...
!$OMP           Int_Factor,                                             &
!$OMP           ULM_LENGTH,                                             &
!$OMP           CC_Ylm_Term, Lagrange_Poly_Table,                       &
!$OMP           M_VALUES,                                               &
!$OMP           Matrix_Location,                                        &
!$OMP           LM_Location,                                            &
            ...
            DO lpmp_loc = 0,LM_Length - 1
                ...
                DO pd = 1,NUM_P_QUAD_POINTS
                    DO td = 1,NUM_T_QUAD_POINTS
                        ...
                        Ylm_Cross_Terms(1:6) = A_Factors(1:6, td, pd, lpmp_loc)*CC_Ylm_Term(td, pd, lm_loc)
...
!            PRINT*,"myID",myID,"Start_Here_RE",Start_Here_RE,             &
!                   "Start_Here_ND",Start_Here_ND,"SUBSHELL_PROB_DIM",SUBSHELL_PROB_DIM
...
!           PRINT*,"REDUCE AFTER  BLOCK_ELEM_STF_MAT",myID
!           PRINT*,BLOCK_ELEM_STF_MATVEC
!           PRINT*,"REDUCE AFTER BLOCK_RHS_VECTOR",myID
!           PRINT*,BLOCK_RHS_VECTOR
...
!                                                                                   !
!###################################################################################!
SUBROUTINE REDUCE_3D_NONLAPLACIAN_SOE_B()
...
INTEGER(KIND = 4), DIMENSION(0:NUM_SUBSHELLS_PER_SHELL-1)     ::  Mat_Requests,           &
                                                                  Vec_Requests

INTEGER(KIND = 4), DIMENSION(MPI_STATUS_SIZE, 0:NUM_SUBSHELLS_PER_SHELL-1)     ::  Mat_Status,           &
                                                                                   Vec_Status
...
!                   "Start_Here_ND",Start_Here_ND,"SUBSHELL_PROB_DIM",SUBSHELL_PROB_DIM
            ...
            IF ( myID_SUBSHELL == SUBSHELL ) THEN
                ...
                CALL MPI_IREDUCE( MPI_IN_PLACE,                                          &
                                 ...
                                 SUBSHELL,                                              &
                                 POSEIDON_COMM_SHELL,                                   &
                                 Mat_Requests(SUBSHELL),                                &
                                 ierr                                                   )


                CALL MPI_IREDUCE( Block_RHS_VECTOR(Start_Here_ND),                       &
                                 RHS_Receive_Buffer,                                    &
                                 SUBSHELL_PROB_DIM,                                     &
                                 MPI_DOUBLE_COMPLEX,                                    &
                                 MPI_SUM,                                               &
                                 SUBSHELL,                                              &
                                 POSEIDON_COMM_SHELL,                                   &
                                 Vec_Requests(SUBSHELL),                                &
                                 ierr                                                   )


            ELSE

                CALL MPI_IREDUCE( BLOCK_ELEM_STF_MATVEC(0,Start_Here_RE),    &
                                 BLOCK_ELEM_STF_MATVEC(0,Start_Here_RE),    &
                                 SEND_LENGTH,                                           &
                                 MPI_DOUBLE_COMPLEX,                                    &
                                 MPI_SUM,                                               &
        ...
        CALL MPI_WAITALL(NUM_SUBSHELLS_PER_SHELL, Mat_Requests, Mat_Status, ierr )
        CALL MPI_WAITALL(NUM_SUBSHELLS_PER_SHELL, Vec_Requests, Vec_Status, ierr )
...
END SUBROUTINE REDUCE_3D_NONLAPLACIAN_SOE_B
...
!           PRINT*,"AFTER FINISH  BLOCK_ELEM_STF_MAT",myID
!           PRINT*,BLOCK_ELEM_STF_MATVEC
...
!    PRINT*,"Doing FINISH_RHS_VECTOR_BLOCK",myID,                        &
!                            myShell*RAD_ELEMS_PER_SHELL                 &
!                          + myID_SubShell*NUM_R_ELEMS_PER_SUBSHELL 
    ...
    !$OMP           rlocs,                                                  &
    !$OMP           INT_R_WEIGHTS, INT_R_LOCATIONS, Lagrange_Poly_Table,    &
    !$OMP           myShell, myID, myID_SUBSHELL, ierr,                     &
    !$OMP           myID_SHELL,                                             &
    !$OMP           Local_Length,                                           &
    !$OMP           ULM_LENGTH,                                             &
    !$OMP           M_VALUES,                                               &
...
!    Start_Here_b = mySHELL*NUM_R_ELEMS_PER_BLOCK*DEGREE*ULM_LENGTH               &
!                 + myID_SUBSHELL*NUM_R_ELEMS_PER_SUBSHELL*DEGREE*ULM_LENGTH
!    End_Here_b = Start_Here_b + SUBSHELL_Prob_Dim -1


!   DO i = 0,NUM_SUBSHELLS
!      IF (myID_PETSC == i) THEN

!         PRINT*,"myID_Petsc",myID_PETSC,myID_SHELL,myID_SUBSHELL,"Start/End",Start_Here,End_Here
!         PRINT*,"Block_STF_MAT"
!         PRINT*,Block_STF_MAT
!         PRINT*," "
!         PRINT*,"Coefficient_Vector",Start_Here_b,End_Here_b
!         PRINT*,Coefficient_Vector(Start_Here_b:End_Here_b)
!         PRINT*,"TMP_VECTOR",myID
!         PRINT*,TMP_VECTOR
!         PRINT*," "
!         PRINT*,"Block_RHS_Vector"
!         PRINT*,Block_RHS_Vector(Start_Here:End_HEre)
...
!                                          + TMP_VECTOR(0:Local_Length-1)
                                          + TMP_VECTOR(0:SUBSHELL_PROB_DIM-1)

    timec=MPI_Wtime()

!    PRINT*,"Block_STF_MAT Create time :",timeb-timea
!    PRINT*,"ZGBMV time                :",timec-timeb
...
!           PRINT*,"AFTER FINISH BLOCK_RHS_VECTOR",myID
!           PRINT*,BLOCK_RHS_VECTOR
...
!                Value_Location =  NUM_R_ELEMS_PER_BLOCK*DEGREE*ULM_LENGTH + NUM_CFA_VARS*(l*(l+1)+m) + ui-1
...
!         RHS_VECTOR(DEGREE+ULM_LENGTH+ui) = 2.0_idp*sqrt(pi)*OUTER_CFA_BC_VALUES(ui)
!         PRINT*,2.0_idp*sqrt(pi)*OUTER_CFA_BC_VALUES(ui)
                !*!
                !*!     Modify the Stiffness Matrix !
                !*!
...
COMPLEX(KIND = idp), DIMENSION(1:2+DOMAIN_DIM)                  ::  Tmp_U_Value
