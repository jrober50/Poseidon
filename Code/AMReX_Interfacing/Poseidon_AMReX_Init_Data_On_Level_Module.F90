...
!        PRINT*,Level,re,te,pe,rd,Cur_R_Locs(rd),E
...
!                                                                       !
 !#####################################################################!
SUBROUTINE Poseidon_Init_Data_On_Level_B( Level,          &
                                        BLo, BHi,       &
                                        SLo, SHi,       &
                                        nComps,         &
                                        Src             )
...
INTEGER                                 ::  re, te, pe
INTEGER                                 ::  rd, td, pd
INTEGER                                 ::  Here, There, Yonder
INTEGER, DIMENSION(3)                   ::  CP
INTEGER                                 ::  Num_DOF

REAL(idp), DIMENSION(1:Num_R_Quad_Points)   ::  Lag_Poly
REAL(idp), DIMENSION(1:Num_R_Quad_Points)   ::  Crs_rlocs
REAL(idp), DIMENSION(1:Num_R_Quad_Points)   ::  Fne_rlocs
...
DO re = BLo(1),BHi(1)
    ...
    Fne_rlocs(:) = (Level_dx(Level,1)/2.0_idp) * (Int_R_Locations(:) + 1.0_idp + re*2.0_idp)
    Crs_rlocs(:) = (Level_dx(0,1)/2.0_idp) * (Int_R_Locations(:) + 1.0_idp + CP(1)*2.0_idp)
    ...
    DO rd = 1,Num_R_Quad_Points
        ...
        Lag_Poly = Lagrange_Poly(Fne_rlocs(rd), Num_R_Quad_Points-1, Crs_rlocs)

        here = (pd-1)*Num_T_Quad_Points*Num_R_Quad_Points   &
             + (td-1)*Num_R_Quad_Points                     &
             + 1
        there = (pd-1)*Num_T_Quad_Points*Num_R_Quad_Points  &
              + (td-1)*Num_R_Quad_Points                    &
              + Num_R_Quad_Points
        Yonder = (pd-1)*Num_T_Quad_Points*Num_R_Quad_Points &
               + (td-1)*Num_R_Quad_Points                   &
               + rd


!        PRINT*,Lag_Poly
!        PRINT*,SUM(Block_Source_E(Here:There,CP(1),CP(2),CP(3))*Lag_Poly(:))

        Src(re,te,pe,0*Num_DOF+Yonder) = SUM(Block_Source_E(Here:There,CP(1),CP(2),CP(3))*Lag_Poly(:))
        Src(re,te,pe,1*Num_DOF+Yonder) = SUM(Block_Source_S(Here:There,CP(1),CP(2),CP(3))*Lag_Poly(:))
        Src(re,te,pe,2*Num_DOF+Yonder) = SUM(Block_Source_Si(Here:There,CP(1),CP(2),CP(3),1)*Lag_Poly(:))
        Src(re,te,pe,3*Num_DOF+Yonder) = SUM(Block_Source_Si(Here:There,CP(1),CP(2),CP(3),2)*Lag_Poly(:))
        Src(re,te,pe,4*Num_DOF+Yonder) = SUM(Block_Source_Si(Here:There,CP(1),CP(2),CP(3),3)*Lag_Poly(:))
...
END SUBROUTINE Poseidon_Init_Data_On_Level_B
